...
export function FocusNow({ locale }: FocusNowProps) {
  ...
  return (
    <section className="interactive-elevated rounded-[2rem] border border-[var(--line)] bg-[var(--card)] p-6 shadow-[0_20px_80px_-60px_rgba(15,118,110,0.45)] md:p-8">
      ...
      <ul className="mt-8 grid gap-4 md:grid-cols-3">
        {content.bullets.map((item) => (
          <li
            key={item}
            className="interactive-elevated rounded-[1.5rem] border border-[var(--line)] bg-[var(--surface)] p-5 text-sm leading-7 text-[var(--foreground)] md:text-base"
          >
